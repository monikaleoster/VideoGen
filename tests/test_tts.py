...
async def test_step_blocks_until_approved() -> None:
    task = asyncio.create_task(run_tts(TtsInput(notes=["note 1", "note 2"])))
...
async def test_step_resumes_and_completes_after_approval() -> None:
    task = asyncio.create_task(run_tts(TtsInput(notes=["note 1", "note 2", "note 3"])))
    ...
    assert len(output.audio_paths) == 3
    assert len(output.durations_sec) == 3
